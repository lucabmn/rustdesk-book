...
-- Backfill: every pre-existing entry targeted a device. Keep the assignment
-- and snapshot the labels that are still resolvable today.
UPDATE "audit_log" SET "target_type" = 'device', "target_id" = "device_id"::text WHERE "device_id" IS NOT NULL;--> statement-breakpoint
